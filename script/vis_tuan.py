...
def create_slalom_gates_from_poles(coordinates, dot_distance=1.5):
    """Create two white dots each dot_distance away from every slalom pole.

    Looks up slalom_pole_0, slalom_pole_1, slalom_pole_2. For each pole, the
    two dots sit perpendicular to that pole's own yaw, dot_distance on either
    side of the pole (so 2*dot_distance apart, straddling the pole).
    """
    pole_names = ["slalom_pole_0", "slalom_pole_1", "slalom_pole_2"]
    poles = {c["name"]: c for c in coordinates if c["name"] in pole_names}

    gates = []
    for name in pole_names:
        pole = poles.get(name)
        if pole is None:
            print(f"Warning: '{name}' coordinate not found")
            continue

        yaw_rad = math.radians(pole.get("yaw", 0.0))
        center_north = pole["x"]
        center_east = pole["y"]

        perp_yaw_rad = yaw_rad + math.pi / 2

        left_north = center_north + dot_distance * math.cos(perp_yaw_rad)
        left_east = center_east + dot_distance * math.sin(perp_yaw_rad)
        right_north = center_north - dot_distance * math.cos(perp_yaw_rad)
        right_east = center_east - dot_distance * math.sin(perp_yaw_rad)
...
def plot_ned_coordinates_with_obstacles(
    ...
    # Slalom pole gate dots (two white dots 1.5m from each pole)
    slalom_gates = create_slalom_gates_from_poles(coordinates, slalom_dot_distance)
    for gate in slalom_gates:
        ...
        ax.plot(
            ...
            markeredgewidth=1,
            label="Slalom Gate Dots" if gate == slalom_gates[0] else "",
            zorder=7,
        )
